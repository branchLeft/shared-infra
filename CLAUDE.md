...
- **A clean `pulumi preview` is evidence about Pulumi state, not about the edge.** For Cloud Armor especially — the provider writes rules as independent per-priority API calls — `deploy-apply` re-captures the live policy and fails by name on any divergence from the committed baseline (`scripts/assert-cloud-armor-live-parity.py`), rather than relying on a human to re-run `gcloud compute security-policies describe`. `RUNBOOK-edge-state-move.md` appendix A is the incident.

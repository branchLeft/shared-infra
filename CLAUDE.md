...
- **CI applies the edge stack and the mail stack on merge to `main`** (`.github/workflows/ci.yml`). Do not hand-apply either. Each apply is a plan job whose `pulumi preview` output lands in the job summary, then an apply job paused by the `production` environment's required-reviewer rule until a human has read that plan — the delete guards catch the destruction class mechanically; the reviewer catches the plan that is green and still wrong. The edge stack's guardrails, its three CI-cannot-apply classes (project IAM, its own federation, deleting a certificate) and their targeted-apply recipe: `RUNBOOK-ci-bootstrap.md`. Mail's delete guard is `scripts/assert-no-hetzner-deletes.py`; its first-time import procedure (not the steady state) is `mail/RUNBOOK-import-mail-host.md`.
- **`hetzner/`'s two stacks (`branchleft-hetzner-network`, `branchleft-hetzner-estate`) do not have their CI apply path yet.** They join the same plan/approve/apply pattern once the estate's hcloud project move lands ([branchLeft/workspace#192](https://github.com/branchLeft/workspace/issues/192)); until then CI type-checks and tests them, holds no credential for them, and their `pulumi up` is a platform-owner bootstrap action per `hetzner/RUNBOOK-new-stack.md` — first-time provisioning, not a steady-state mechanism.

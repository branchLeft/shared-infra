...
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'chmod +x /root/mail-provision/*.sh /root/mail-provision/*.py && /root/mail-provision/run-all.sh'
```
...
```bash
scp -i ~/.ssh/id_ed25519_hetzner -r mail/provision/. root@mx1.branchleft.co.uk:/root/mail-provision
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'chmod +x /root/mail-provision/*.sh /root/mail-provision/*.py && /root/mail-provision/62-provision-shim-submission-credential.sh && /root/mail-provision/63-deploy-mailgun-shim.sh'
```
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'docker compose -p mailgun-shim -f /etc/mailgun-shim/shim-compose.yml exec mailgun-shim node dist/cli.js register blog.branchleft.co.uk'
```
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'vi /var/lib/mailgun-shim/throttle.json'
```

### Log access

```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'docker compose -p mailgun-shim -f /etc/mailgun-shim/shim-compose.yml logs -f mailgun-shim'
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'docker compose -p mailgun-shim -f /etc/mailgun-shim/shim-compose.yml logs -f caddy'
```
...
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/check_dnsbl_blocklist.py --status'

# Force a fresh live check right now:
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/check_dnsbl_blocklist.py'

# Anything the automated hourly run has flagged as needing attention:
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'journalctl -t dnsbl-check -p err -b'
```
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/rotate_admin_credential.py'
```
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner -L 18080:127.0.0.1:8080 root@mx1.branchleft.co.uk
# then, on the host:
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/list_and_clear_blocked_ips.py --list-only'
# prints one line per blocked IP, e.g.:
#   198.51.100.7    reason=portScanning    since=2026-08-11T22:50:18Z

ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/list_and_clear_blocked_ips.py --ip 198.51.100.7'
...
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/list_and_clear_blocked_ips.py --reason portScanning'
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 <<EOFPYTHON
import json
...
recipient's `Authentication-Results` header. This check happens only when
demo-domain mail first flows through the host — a local test send via the
provision scripts does not exercise DKIM signing, only the delivery
forwarding logic verified earlier. Real DKIM validation requires a message
...
```bash
ssh -i ~/.ssh/id_ed25519_hetzner root@mx1.branchleft.co.uk 'python3 /root/mail-provision/trigger_acme_renewal.py'
```

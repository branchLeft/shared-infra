...
and an API-driven DNS zone. If those land in the estate project, one leaked
estate token reaches customer data, strangers' demo code and DNS at once.

The design (try-it-now §02, and the DNS decision in the mail-delivery design)
fixes five projects:
...
| `dns`     | `branchLeft dns`     | new, empty: the DNS zone later, never a host        |
...
are not touched by any step here.

Where the Hetzner backup bucket lives is still an open design question
(branchLeft/workspace#1203). This runbook creates no bucket.
...
### The ProtonPass naming convention (all five projects, from now on)

One entry per token, titled `hcloud / <project> / <token name>`, where
`<project>` is one of `mail`, `org`, `tenants`, `demos`, `dns` and `<token
name>` is the token's name in the Console, exactly. The entry holds:
...
### 1. Create the three new projects

Hetzner Console → **Projects** → **+ New project**, three times:
`branchLeft tenants`, `branchLeft demos`, `branchLeft dns`.

Expected: five projects on the Projects page, plus any lab project.

### 2. Write down all five project IDs

Open each project. Its ID is the number in the address bar,
`console.hetzner.com/projects/<ID>/...`. Keep the five `name → ID` pairs for
step 7. Project IDs aren't secret.

Expected: five distinct numbers. The mail project and the estate project are
two different IDs. If you only find one, stop: the estate is not where this
runbook thinks it is.

### 3. Create the five marker firewalls

In **each** of the five projects: **Firewalls** → **Create Firewall**.

- Name: `project-marker-mail`, `project-marker-org`,
  `project-marker-tenants`, `project-marker-demos`, `project-marker-dns`,
  one per project, matching the table at the top.
...
Expected: five ProtonPass entries matching the table, and each project's API
...
the cursor waits. Nothing is echoed.
...
```bash
unset HCLOUD_PROBE_TOKEN_MAIL HCLOUD_PROBE_TOKEN_ORG HCLOUD_PROBE_TOKEN_TENANTS HCLOUD_PROBE_TOKEN_DEMOS HCLOUD_PROBE_TOKEN_DNS
```

### 7. Record the project-ID map

Paste the five `name → ID` pairs from step 2, and the full output of steps 5
...
- Later stories, not this one: register the owner SSH key
  ...
  create-time-only. Add `HCLOUD_TOKEN_TENANTS`/`_DEMOS`/`_DNS` as repository
  secrets when those stacks first get CI.

...
export function renderAlertRules(): string {
  return `${[
    ...
    '            yields no series at all, not a large number. Two different causes',
    '            share this one alert, and the response differs: an expired',
    '            SNDS_BEARER_TOKEN (its ~8h life is well inside this 36h window) is',
    '            the routine case, cleared by renewing it (RUNBOOK-monitoring.md',
    '            §14, branchLeft/workspace#538) -- only treat this as a bug if a',
    '            current, recently-renewed token still fails to produce a fresh',
    '            snapshot.',
  ].join('\n')}\n`;
}
